 ...
 * analysis. Excluded entirely (not prose): code fences and their contents,
 * tables, headings, horizontal rules, image-only lines, HTML comments,
 * YAML frontmatter. Kept lines have list/blockquote/emphasis markers and
 * link URLs stripped (link text survives).
 *
 * @param {string} text
...
export function proseLines(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^\|.*\|/.test(trimmed) || /^[|:\s-]+$/.test(trimmed)) continue; // tables / rules
...
const NEGATION_ANTITHESIS_RES = [
  ...
  // "not X, but Y" pivot within one sentence
  /\bnot\s+(?:about\s+|just\s+|only\s+)?[^.!?,]{2,60},\s*but\b/i,
];
...
function findNegationAntithesis(keptLines) {
  ...
  for (let i = 0; i < keptLines.length; i++) {
    ...
    for (let j = i + 1; j < keptLines.length && keptLines[j].text; j++) {
      windowText += ' ' + keptLines[j].text;
      break;
    }
...
function findAcronyms(keptLines) {
  ...
  for (const { line, text } of keptLines) {
    ...
    for (const m of text.matchAll(ACRONYM_RE)) {
      ...
      // defined if the first use sits inside parentheses right after prose
      // ("Federal Election Commission (FEC)") or is itself followed by a
      // parenthetical expansion ("FEC (Federal Election Commission)")
      const idx = m.index ?? 0;
      const before = text.slice(Math.max(0, idx - 2), idx);
      const after = text.slice(idx + m[0].length, idx + m[0].length + 2);
      const defined = before.endsWith('(') || after.startsWith(' (') || after.startsWith('(');
...
function findKillListHits(keptLines, killList) {
  ...
  for (const term of terms) {
    const re = new RegExp(`\\b${term.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'gi');
    const lines = [];
    let count = 0;
    for (const { line, text } of keptLines) {
      if (!text) continue;
    ...
    if (count > 0) hits.push({ word: term, count, lines });
  }
  return hits;
}
...
export function analyzeReadability(text, opts = {}) {
  const audience = AUDIENCES.includes(opts.audience) ? opts.audience : 'default';
  ...
  let sentenceCount = 0;
  let syllableCount = 0;
  let maxSentence = { words: 0, line: 0, text: '' };
  ...
  for (const p of paragraphs) {
    for (const s of p.sentences) {
      ...
      if (ws.length > maxSentence.words) {
        maxSentence = { words: ws.length, line: p.line, text: s.slice(0, 160) };
      }
  ...
  return {
    ...
    tricolon_paragraphs: findTricolons(paragraphs),
    negation_antithesis: findNegationAntithesis(kept),

...
export function resolveArtifact(runDir, state, explicit) {
  ...
  // long-form runs put several deliverable-adjacent .md files at the run root
  // (moderator_report.md, locked_plan.md, stitch_report.md, report.raw.md);
  // report.md is the deliverable when present
  if (rootMds.includes('report.md')) return join(runDir, 'report.md');
  return rootMds.length === 1 ? join(runDir, rootMds[0]) : null;
}

/**
 * Last logged artifact size: the final row's total_lines column when present,
 * else the state.json line-count field. Shared by the full check and
 * preflight so the two gates can never disagree about what was logged.
 *
 * @param {object[]} rows
 * @param {string[]} columns
 * @param {object|null} state
 * @returns {number|null}
 */
function resolveLoggedLines(rows, columns, state) {
  if (rows.length > 0 && columns.includes('total_lines')) {
    const n = toNumber(rows[rows.length - 1].total_lines);
    if (n !== null) return n;
  }
  return toNumber(state?.skill_line_count ?? state?.artifact_line_count);
}
...
export function checkRunConsistency(runDir, opts = {}) {
  ...
  const loggedLines = resolveLoggedLines(rows, columns, state);
  ...
  if (rows.length >= 3 && columns.includes('decision') && columns.includes('delta')) {
    ...
    for (const r of rows) {
      ...
      // recompute each row (never latch): a transient early plateau the run
      // recovered from needs no override entry — only a streak that is still
      // live at the end of the ledger does
      signal = plateauRun >= 3
        ? 'plateau (3+ consecutive keeps with delta < 0.1)'
        : revertRun >= 3
...
export function preflightCheck(runDir, opts = {}) {
  ...
  const expectedLines = resolveLoggedLines(results?.rows ?? [], results?.columns ?? [], state);
  ...
  // A ledger with rows but no logged line count or hash cannot actually be
  // verified — surface that instead of passing silently (a fresh run with no
  // rows yet legitimately has nothing to compare and stays verifiable=false
  // without warning).
  const unverifiable = expectedLines === null && expectedSha === null && (results?.rows?.length ?? 0) > 0;

...
// selfinvestigate.md. Uses the same character-bigram Jaccard similarity the
// selfpost duplicate check already relies on.
//
...
export function parseRecords(text, isJsonl) {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      ...
      return { records: Object.entries(parsed).map(([k, v]) => ({ id: k, ...(v ?? {}) })) };
    }

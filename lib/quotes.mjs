...
export function elisionFragments(quoteText) {
  const parts = String(quoteText ?? '')
    ...
    .filter((f) => f.length > 0);
  // The word/length floor only guards against tiny leftover fragments of an
  // actual elision split. An unsplit quote is checked whole, however short —
  // a verbatim two-word quote must never be reported as fabricated.
  if (parts.length <= 1) return parts;
  return parts.filter((f) => f.split(' ').filter(Boolean).length >= MIN_FRAGMENT_WORDS || f.length >= 20);
}
 ...
 */
function parseSources(sourcesJsonText) {
  let parsed;
  try {

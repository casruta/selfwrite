...
import { jaccardBigramSimilarity } from './similarity.mjs';

// Compatibility verifier for pre-v3 runs. Discovery snippets and generated
// TLDR fields are deliberately excluded; v3 uses documents/ + evidence.jsonl.
const SOURCE_TEXT_FIELDS = Object.freeze(['verbatim_source_text', 'abstract']);
 ...
 * retaining every non-empty fragment, including one-word fragments.
 *
 * @param {string} quoteText
 * @returns {string[]} normalized fragments
 */
export function elisionFragments(quoteText) {
  const parts = String(quoteText ?? '')
    .split(/\[(?:…|\.{3,})\]|…|\.{3,}/)
    .map(normalizeForMatch)
    .filter((f) => f.length > 0);
  // Every fragment is binding: dropping a short fragment can make an altered
  // quote appear verbatim (for example, "not ... guilty").
  return parts;
}
 ...
 * (stored original text or a publisher-provided abstract).
 *
 * @param {{ quote_text?: string }} quote
 * @param {{ verbatim_source_text?: string, abstract?: string }} source
...
export function verifyQuote(quote, source) {
  ...
  if (fragments.length > 1 && fragments.some((fragment) => fragment.split(/\s+/).length < 2)) {
    return { verbatim: false, reason: 'elision_fragment_too_short' };
  }
...
export function parseSources(sourcesJsonText) {
  ...
  if (records) {
    for (const r of records) {
      ...
      if (typeof id === 'string') {
        if (sources.has(id)) duplicate_ids.push(id);
        else sources.set(id, r);
      }
    }
...
export function verifyQuotesFile(quotesJsonlText, sourcesJsonText) {
  const base = {
    ...
    duplicate_source_ids: [], duplicate_quote_ids: [],
  };

  const { sources, error, duplicate_ids = [] } = parseSources(sourcesJsonText);
  if (!sources) return { ...base, ok: false, pass: false, error };
  base.duplicate_source_ids.push(...duplicate_ids);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (seenQuoteIds.has(quoteId)) base.duplicate_quote_ids.push(quoteId);
    else seenQuoteIds.add(quoteId);
  ...
  base.pass = base.fabricated.length === 0 && base.parse_errors.length === 0 &&
    base.duplicate_source_ids.length === 0 && base.duplicate_quote_ids.length === 0;
  return base;
}

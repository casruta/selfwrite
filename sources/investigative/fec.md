...
No API key required for basic use, but requests without a key are rate-limited aggressively. Get a free key at `api.data.gov/signup/` and append `&api_key=<key>` to every request — raises the quota to 1000 req/hour.
...
```
GET /candidates/search/?q=<name>&api_key=<key>
```
...
```
GET /schedules/schedule_a/?contributor_name=<name>&two_year_transaction_period=<year>&per_page=100&api_key=<key>
```
...
```
GET /committees/?q=<name>&api_key=<key>
```
...
```
GET /schedules/schedule_b/?committee_id=<id>&per_page=100&api_key=<key>
```
...
```
GET /schedules/schedule_e/?support_oppose_indicator=<S|O>&candidate_id=<id>&api_key=<key>
```
...
```
GET /schedules/schedule_a/by_employer/?employer=<string>&api_key=<key>
```
...
```
WebFetch(
  url="https://api.open.fec.gov/v1/schedules/schedule_a/?contributor_name=Adelson&two_year_transaction_period=2020&per_page=100&api_key=<key>",

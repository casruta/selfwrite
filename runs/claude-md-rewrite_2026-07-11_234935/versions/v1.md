...
- `selfwrite.md` / `selfresearch.md` / `selfinvestigate.md` — the skill prompts.
- `lib/` — pure logic with errors returned as values; nothing throws, no default exports.
- `scripts/` — thin CLIs over `lib/`; each takes `--json` and exits 0 on pass, 1 on fail, 2 on input error. The exception is `near-dupes.mjs`: it is advisory-only and exits 0 on any successful run, since near-dupe pairs are merge-or-keep judgment calls for the skill.
- `sources/` — backend reference cards the research skills read at runtime.
- `config/` — holds `kill-list.yaml`, the canonical AI-tell word list.
- `runs/` — real run artifacts and read-only test fixtures; never rewrite them.
- `ANALYSIS.md` — the audit the invariants below come from.

## Invariants (enforced by scripts, verified by tests)

- **Run ledger.** `results.tsv` gets one row per ATTEMPTED iteration, reverts included. `# schema_version: 2` sits atop `results.tsv` and inside `state.json`. The artifact is never edited outside the logged loop: preflight runs before every DRAFT, a full check runs at every REFLECT and at run end, and error-level findings block the next iteration.

  ```bash
  node scripts/run-integrity.mjs <run_dir> --preflight
  ```

- **Citations.** Every quote must be a verbatim substring of its stored source text. The check below must pass before VERIFY completes, and a `fabricated` entry FAILs with no semantic appeal.

  ```bash
  node scripts/verify-quotes.mjs <run_dir>
  ```

- **Readability gate.** FK grade <= 12.0 for the default audience and <= 10.0 for a general audience. Average sentence <= 20 words (17 for general), with no sentence over 35 words. An `expert` audience is exempt from the cap; stats are still logged. Thresholds live in `THRESHOLDS` in `lib/readability.mjs`, and FK numbers stated in skill prose must match them (lint-enforced; the sentence-length numbers are convention only).
